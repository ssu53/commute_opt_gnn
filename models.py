...
class GINVanilla(nn.Module):
    ...
    def __init__(self, in_channels: int, hidden_channels: int, num_layers: int, out_channels: int, eps: float = 0.0, bias: bool = True, pool_all: bool = True):
        """
        Args        
            ...
            pool_all: pool features across all nodes (for e.g. graph-level tasks)
        """
...
class GINExpander(nn.Module):
    ...
    def __init__(self, in_channels: int, hidden_channels: int, num_layers: int, out_channels: int, eps: float = 0.0, bias: bool = True, pool_all: bool = True):
        ...
        TODO
            possibly replace Linear with full MLP in GINConv?
        """
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor, expander_edge_index: torch.Tensor):
        """
        Args
            x: input
            edge_index: input graph edge index of shape (2, num_edges)
